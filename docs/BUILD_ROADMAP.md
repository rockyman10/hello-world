...
> **Status: started.** The prototype now includes the full core loop — missions → Voidglass → gacha (exact pity math: soft 74 / hard 90 / 50-50 / per-10 4★ guarantee) → roster/squad building → gear sets with speed tuning → harder encounters, with localStorage saves. Also in: character leveling (Credits sink, cap 30), the idle Meridian Reactor (Credits accrue offline, 24h cap), and the Void Rift roguelite (3 depths, blessing draft, wounds persist, rewards kept on defeat). Meta logic is DOM-free in `prototype/meta.js`; the test harness verifies pull rates, both pity guarantees, encounter win-rate bands, and all three systems.
